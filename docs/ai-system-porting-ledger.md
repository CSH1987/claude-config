...
| `PORT-005` | 압축 전 중요 상태를 보존하고 압축 뒤 정본을 복원한다 | `compact_prompt` + `PreCompact`·`PostCompact` 경계 알림 + `SessionStart(source=compact)` · `partial` | Vault 정본·로컬 memory 정책 · `policy_only` | `2026-08-21 / 2026-08-21 / Codex 0.148.0 / 수동·자동·오류 입력 직접 훅 시험, config merge test, test/codex-integration.sh` | 실제 모델 보존은 `compact_prompt`, 복원은 SessionStart가 담당하고 Pre/Post의 `systemMessage`는 UI 경고일 뿐 모델 컨텍스트가 아니다. 실제 장기 세션 압축 E2E와 새 훅 신뢰 확인은 남음. 기본 압축 설정·훅 비활성화로 복구 |
...
| `PORT-008` | 계획·검증·구현·대량 탐색을 분리한다 | native subagents + 4개 등록 역할 + `workload-optimization` · `partial` | skill + routing self-review · `partial` | `2026-08-21 / 2026-08-21 / Codex 0.148.0, Hermes 0.20.4 / 역할 등록 strict load, 프로필별 실제 `codex exec`, 설정·배포 회귀` | Sol·Terra·Luna 프로필과 역할 등록은 확인했지만 비대화형 실행에서 모델이 역할 호출을 완료하지 않아 native subagent E2E와 업무 표본의 품질·비용 측정은 남음. 품질이 흔들리면 Sol 단일 역할로 복귀 |
...
| `PORT-017` | 승인된 로컬 작업은 매번 권한 질문 없이 수행하고 즉시 안전 모드로 복구할 수 있다 | 전역 `approval_policy=never` + `sandbox_mode=danger-full-access`, 치명적 명령 `PreToolUse` 가드레일, `safe` 프로필 · `partial` | 이번 적용 범위 밖 · `unverified` | `2026-08-21 / 2026-08-21 / Codex 0.148.0 / strict doctor 18개 정상, 기본·safe 실제 `codex exec`, 가드레일 직접 deny/allow, 설치·멱등·충돌 백업 회귀` | 전체 접근과 safe 복구는 검증됐지만 가드레일의 Codex 런타임 신뢰는 사용자 확인 전이다. 가드레일은 협조적 에이전트의 실수 방지용 fail-open 보조장치이며 보안 경계가 아니다. 외부 발송·일반 삭제·결제는 공통 지침에 의존한다 |
...
1. Codex `/hooks`에서 새 `PreToolUse`·`PreCompact`·`PostCompact`를 신뢰한 뒤 실제 장기 세션의 `/compact` 전후 복구를 확인
