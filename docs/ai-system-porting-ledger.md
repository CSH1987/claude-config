...
| `PORT-004` | 세션 시작·종료에 결정적 작업을 실행한다 | `SessionStart`·`SessionEnd` command hook · `implemented_unverified` | 공통 규칙 로드와 로그 수집 경로 · `partial` | `2026-08-21 / 2026-08-21 / Codex 0.148.0, Hermes 미실행 / test/codex-integration.sh` | Codex `/hooks` 신뢰 전 훅이 건너뛰어질 수 있음. 훅 비활성화로 복구. hook schema 변경 때 재검증 |
...
| `PORT-009` | Claude·Hermes·Codex 대화와 Vault 변경분을 증분 학습한다 | 루트 사용자 발화 수집 · `verified` | export 수집 경로 · `implemented_unverified` | `2026-08-21 / 2026-08-21 / N/A(런타임 비의존 fixture), 소스는 이 대장 도입 commit / test/learning-pipeline-sources.sh` | 구형 Claude+Hermes 파이프라인은 3회 성공했지만 새 3-source/v2 파이프라인 실제 실행은 0회. launchd 중지·커서 복원으로 rollback. 새 파이프라인 첫 성공·export schema 변경 때 재검증 |
| `PORT-010` | 세션 종료 후 재개 단서를 남긴다 | `SessionEnd` → 공용 logger · `implemented_unverified` | 주간 수집 및 Vault 기록 · `partial` | `2026-08-21 / 미실행 / Codex 0.148.0, Hermes 0.20.4 / 종료 훅·설정 검토` | Codex 훅 신뢰 전 로그 누락 가능. 훅 해제 가능. 첫 실제 종료 이벤트 때 재검증 |
...
1. Codex `/hooks`에서 현재 해시 신뢰 승인 후 실제 SessionStart·SessionEnd 확인
2. 새 3-source/v2 주간 학습 파이프라인의 최초 성공 실행과 Vault 비파괴 갱신 확인(구형 Claude+Hermes 파이프라인은 3회 성공 이력 있음)
