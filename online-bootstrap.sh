...
사용법: bash online-bootstrap.sh [옵션]
  ...
  --with-hermes            이 머신에도 Hermes를 설치/연결
  --without-hermes         Hermes 설치/연결 생략
  --skip-install           누락 앱/CLI를 설치하지 않고 상태만 보고
  --no-open                완료 후 Obsidian을 열지 않음
  -h, --help               이 도움말

Windows는 기존 bootstrap.ps1을 사용한다. Hermes는 기존 운영 원칙상 Mac mini
오케스트레이터에만 자동 적용하며, 다른 머신에서는 --with-hermes가 있어야 설치한다.
EOF
}
...
if [ "$HERMES_MODE" = "auto" ]; then
  if command -v hermes >/dev/null 2>&1 || [ -d "$HOME/.hermes" ] || is_macmini_host; then
    HERMES_MODE="yes"
  else
    HERMES_MODE="no"
  fi
...
_repo_info="$(gh repo view "$VAULT_REPO" --json nameWithOwner,visibility --jq '.nameWithOwner + "|" + .visibility' 2>/dev/null || true)"
_repo_name="${_repo_info%%|*}"
_repo_visibility="${_repo_info#*|}"
[ -n "$_repo_info" ] && [ "$_repo_info" != "$_repo_visibility" ] \
...
ok "비공개 Vault 저장소 확인: $VAULT_REPO"
...
if [ -d "$VAULT_DIR/.git" ]; then
  ...
  _origin_info="$(gh repo view "$_origin" --json nameWithOwner,visibility --jq '.nameWithOwner + "|" + .visibility' 2>/dev/null || true)"
  _origin_name="${_origin_info%%|*}"
  _origin_visibility="${_origin_info#*|}"
...
python3 - "$SCOPE_FILE" "$VAULT_DIR" <<'PY'
import json
import os
import sys
import tempfile
...
if [ "$HERMES_MODE" = "yes" ]; then
  if ! install_cli_if_missing hermes "Hermes Agent" hermes; then
    ...
    python3 - "$HERMES_ENV" "$VAULT_DIR" <<'PY'
import os
import sys
import tempfile
...
PY
  ...
  info "Hermes는 이 머신에서 생략했습니다(기존 원칙: Mac mini 오케스트레이터 전용; 필요하면 --with-hermes)."
fi
...
if [ "$HERMES_MODE" = "yes" ]; then
  if command -v hermes >/dev/null 2>&1 && [ -f "$HOME/.hermes/config.yaml" ]; then
    ok "Hermes 설치/설정 확인"
  else
    warn "Hermes 설치/설정 확인 실패"
    INCOMPLETE=1
  fi
fi

if [ -f "$HOME/.codex/AGENTS.md" ] && [ "$(grep -cF '<!-- claude-config:portable-rules:start -->' "$HOME/.codex/AGENTS.md" 2>/dev/null || true)" = "1" ]; then
  ok "Codex 공통 지침 마커 1개"
else
  warn "Codex 공통 지침 마커 검증 실패"
  INCOMPLETE=1
fi
if [ "$HERMES_MODE" = "yes" ]; then
  if [ -f "$HOME/.hermes/AGENTS.md" ] && [ "$(grep -cF '<!-- claude-config:portable-rules:start -->' "$HOME/.hermes/AGENTS.md" 2>/dev/null || true)" = "1" ]; then
    ok "Hermes 공통 지침 마커 1개"
  else
    warn "Hermes 공통 지침 마커 검증 실패"
    INCOMPLETE=1
  fi
fi
...
if [ "$INCOMPLETE" -eq 0 ]; then
  ...
  info "Obsidian Git과 GitHub 인증을 사용하므로 Obsidian Sync를 중복 연결하지 않습니다."
  exit 0
fi
